...
export default function MyOrders() {
  ...
  const filteredOrders = orders.filter(order => {
    if (!order || !order.items) return false; // Handle cases where order or items are undefined
          ...
          <div key={order.id} className="bg-agri-gray rounded-lg relative">
            ...
            <div className="p-3 pt-10 space-y-2">
              ...
              <div className="space-y-1">
                {/* Assuming order.order_items is populated or adjust based on your data structure */}
                {order.order_items && order.order_items.map((item, index) => (
                  <div key={index} className="text-sm">
                    <span className="font-medium text-xs">{item.product_name}</span> {/* Adjust based on actual column name */}
